import { getAdminSignature, getUserSignature } from '@/server-functions/authz'
import type { authorization } from '@onflow/fcl'
...
export function adminAuthorizationFunction(account: any): typeof authorization {
    const address = process.env.ADMIN_ADDRESS!
    const keyIndex = process.env.ADMIN_KEY_INDEX!
    return {
        ...account, // bunch of defaults in here, we want to overload some of them though
        ...
        signingFunction: async (signable: any) => {
            ...
            const { signature } = await getAdminSignature(signable.message)
    ...
    return async function (account: any) {
        ...
        return {
            ...
            signingFunction: async (signable: any) => {
                ...
                const { signature } = await getUserSignature(signable.message, privateKey)
